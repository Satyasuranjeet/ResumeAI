...
def clean_word(word):
    ...
    # Only return words that are alphabetic and have length > 1
...
def extract_keywords(text):
    """Extract keywords from text by removing stopwords and cleaning"""
    if not text:
        return set()
...
def calculate_score(resume_keywords, job_keywords):
    ...
    # Get top missing keywords (sorted by potential importance)
    missing_list = list(missing_keywords)
    suggestions = missing_list[:20]  # Top 20 suggestions
    
    return round(score, 2), list(matched_keywords), suggestions
...
def analyze_resume():
    try:
        ...
        try:
            ...
            if not resume_text:
                return jsonify({
                    'error': 'Could not extract text from the PDF. The file might be corrupted, password-protected, or contain only images.'
                }), 400
            ...
            score, matched_keywords, suggestions = calculate_score(resume_keywords, job_keywords)
            ...
            response_data = {
                ...
                'matched_keywords': matched_keywords[:20],  # Show top 20 matches
